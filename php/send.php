...
$mail = new PHPMailer;
	...
	$usermail = "noreply@motor.bosch-rostov.ru";
    $mail->setFrom($usermail, 'Обработчик почты motor.bosch-rostov.ru');
    ...
    $mail->addReplyTo($sendto,'Landing Page Bosch');
     ...
     $mail->isHTML(true);   
		...
		$siteUrl = "motor.bosch-rostov.ru";
		$subject  = "Новое сообщение - с сайта pro-service-rostov.ru";
		...
		$msg .= '<!--LOGO--><table width="220" align="left" border="0" cellpadding="0" cellspacing="0"><tr><td height="70" style="padding: 0 20px 20px 0;">';
		$msg .= '<!--ТУТ ССЫЛКА НА ЛОГО--><img src="http://'.$siteUrl.'/assets/site/img/logo.png" width="220"  border="0" alt="" / ></td></tr></table><!--END-LOGO-->';
